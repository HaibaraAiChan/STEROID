...
def single_potEnergy(loc1, ld_type_list, mol2_in_string, protein_file):
    ...
    for item in ld_type_list:
        ...
        # print(temp_filename)
        # print(protein_file)
        child = subprocess.Popen(['./dligand-linux', temp_filename, protein_file], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        child.wait()
...
def potEnergy(binding_site, mol2_in_path, protein_file):
    ld_type_list = ['C.2', 'C.3', 'C.ar', 'F', 'N.am', 'N.2', 'O.co2', 'N.ar', 'S.3', 'O.2', 'O.3', 'N.4', 'P.3',
                    'N.pl3']
    total_potE = {'loc': [], 'potE': []}
...
class Grid3DBuilder(object):
    ...
    def build(protein_coords, protein_path, r, N):
        ...
        df2 = pd.DataFrame(total_potE['potE'],
                           columns=['C.2', 'C.3', 'C.ar', 'F', 'N.am', 'N.2', 'O.co2', 'N.ar', 'S.3', 'O.2', 'O.3',
                                    'N.4', 'P.3', 'N.pl3'])
        frames = [df1, df2]
